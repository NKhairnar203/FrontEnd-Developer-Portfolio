const words = ["Aspiring", "Software", "Developer"]; // Array of words to be typed and then deleted in reverse order
...
function typeWords(index) {
  ...
  if (isTyping) {
    typingTextElement.textContent += currentWord.slice(
      currentText.length,
      currentWord.length
    );
    ...
    if (currentText.length > 0) {
      ...
      setTimeout(() => {
        typeWords(index);
      }, 2000); // Pause before typing next word
    }
  }
}
...
btn.addEventListener("click", () => {
  window.open(
    "https://drive.google.com/file/d/1zj6sWuPyXc6RrqQ5U39kganHd7zr7Uam/view?usp=sharing"
  );
});
